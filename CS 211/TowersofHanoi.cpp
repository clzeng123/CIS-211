...
int main() {
    vector<int> t[3]; // three towers A,B,C represened as an array of 3 vector 
    ...
    //ntitialize towers and candidate 
    from = 0;
    to = 1;
    candidate = 1;
    while(t[1].size() < n+1)
    {
        //write it
        cout << "move number " << ++move <<": Transfer ring" 
        << candidate << " from tower " << char(from+65) << " to tower " 
        << char(to+65) << endl;
